...
- Перезапускает **Pod**'ы до успешного выполнения задачи или истечения таймаутов `activeDedLineSeconds` или `backoffLimit`

`activeDedLineSeconds` - максимальное время (в секундах), в течение которого Job может быть активна (выполняться).

- Если время выполнения Job превышает это значение, Kubernetes завершит все связанные Pod'ы и пометит Job как failed.
- Таймер начинается с момента создания Job.

`backoffLimit` - Максимальное количество попыток перезапуска Job после неудачного выполнения Pod'а.

- По умолчанию backoffLimit равен 6.
- После превышения количества неудачных попыток, Job считается failed.
